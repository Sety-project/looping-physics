...
def check_liquidity_constraints(params, user_tvl, sponsor_tvl):
    ...
    # Check N* > 0
    n_star_positive = N_l_star >= 0 and N_d_star > -1e-6
    ...
    # From eq:dex_liquidity_constraint
    constraint_1_ok = abs(N_l_star * U - (N_l + N_l_star) * 0.9) < 1e-6
    constraint_2_ok = abs(N_d_star * alpha - (N_d + N_d_star) * 0.9) < 1e-6
...
def main():
    ...
    r_b = st.sidebar.slider("Borrow Rate at 90% (r_b)", 0.0, 0.30, 0.1, 0.01, help="Borrow rate at 90% utilization")
    ...
    APY_l = st.sidebar.slider("Lending APY Target", 0.10, 0.3, 0.15, 0.01, help="Target APY for lending")
    ...
    if optimization_result['success']:
        ...
        # Show improvement
        current_tvl = user_tvl['N_l'] + user_tvl['N_d'] + user_tvl['N_v']
        improvement = optimization_result['optimal_tvl'] - current_tvl
        improvement_pct = (improvement / current_tvl) * 100 if current_tvl > 0 else 0
        
        st.markdown("**Optimization Improvement:**")
        st.write(f"**Current TVL**: ${current_tvl:,.0f}")
        st.write(f"**Optimal TVL**: ${optimization_result['optimal_tvl']:,.0f}")
        st.write(f"**Improvement**: ${improvement:,.0f} ({improvement_pct:+.1f}%)")
        ...
        if 'constraints_satisfied' in optimization_result:
            ...
            st.write(f"**= {budget_lhs:,.0f}**")
            st.write(f"**Budget RHS**: R = {budget_rhs:,.0f}")
            st.write(f"**Error**: |LHS - RHS| = {budget_error:.6f}")
    ...
    # Show the direct equations from LaTeX
    st.subheader("📝 Direct Equations from LaTeX")
    
    st.markdown("""
    **User TVL relationships:**
    """)
    st.latex(f"N_l = \\frac{{R_l}}{{APY_l - r_b(1-\\epsilon)}}")
    st.write(f"**N_l = {params['R_l']:,} / ({params['APY_l']:.1%} - {params['r_b']:.1%} × {1-params['epsilon']:.2f}) = {user_tvl['N_l']:,.0f}**")
    
    st.latex(f"N_d = \\frac{{R_d}}{{APY_d - \\frac{{r}}{{2}}}}")
    st.write(f"**N_d = {params['R_d']:,} / ({params['APY_d']:.1%} - {params['r']/2:.1%}) = {user_tvl['N_d']:,.0f}**")
    
    st.latex(f"N_v \\leq \\frac{{R_v}}{{APY_v - lr + (l-1)r_b}}")
    max_n_v = R_v / (params['APY_v'] - params['l'] * params['r'] + (params['l']-1) * params['r_b'])
    st.write(f"**N_v ≤ {R_v:,.0f} / ({params['APY_v']:.1%} - {params['l']:.1f} × {params['r']:.1%} + {params['l']-1:.1f} × {params['r_b']:.1%}) = {max_n_v:,.0f}**")
    st.write(f"**Determined by active constraints: N_v = {user_tvl['N_v']:,.0f}**")
    
    st.markdown("""
    **Matrix Solution for N*:**
    """)

    
    st.write(f"**Solution:**")
    st.write(f"**N_l* = {sponsor_tvl['N_l_star']:,.0f}**")
    st.write(f"**N_d* = {sponsor_tvl['N_d_star']:,.0f}**")
    
    st.markdown("""
    **Verification:**
    """)
    st.latex(f"N_l^* + N_d^* = N")
    st.write(f"**{sponsor_tvl['N_l_star']:,.0f} + {sponsor_tvl['N_d_star']:,.0f} = {params['N']:,}**")
    
    st.latex(f"U(N_l+N_l^*) = \\alpha(N_d+N_d^*)")
    st.write(f"**{params['U']:.1f} × ({user_tvl['N_l']:,.0f} + {sponsor_tvl['N_l_star']:,.0f}) = {params['alpha']:.1f} × ({user_tvl['N_d']:,.0f} + {sponsor_tvl['N_d_star']:,.0f})**")
    st.write(f"**{sponsor_tvl['constraint_1']:,.0f} = {sponsor_tvl['constraint_2']:,.0f}**")
    
    st.latex(f"N_v(l-1) = U(N_l+N_l^*)")
    st.write(f"**N_v × {params['l']-1:.1f} = {params['U']:.1f} × ({user_tvl['N_l']:,.0f} + {sponsor_tvl['N_l_star']:,.0f}) = {sponsor_tvl['constraint_1']:,.0f}**")
    st.write(f"**N_v = {user_tvl['N_v']:,.0f}**")
    
    st.markdown("""
    **R_v Calculation:**
    """)
    st.latex(f"R_v = N_v \\max(0, APY_v - lr + (l-1)r_b)")
    apy_diff = params['APY_v'] - params['l'] * params['r'] + (params['l']-1) * params['r_b']
    st.write(f"**R_v = {user_tvl['N_v']:,.0f} × max(0, {params['APY_v']:.1%} - {params['l']:.1f} × {params['r']:.1%} + {params['l']-1:.1f} × {params['r_b']:.1%})**")
    st.write(f"**R_v = {user_tvl['N_v']:,.0f} × max(0, {apy_diff:.1%}) = {R_v:,.0f}**")
    ...
    **Equations:**
    ...
    current_tvl = user_tvl['N_l'] + user_tvl['N_d'] + user_tvl['N_v']
    st.write(f"**Current TVL**: ${current_tvl:,.0f}")
    st.write(f"**R_l**: ${params['R_l']:,}")
    st.write(f"**R_d**: ${params['R_d']:,}")
    st.write(f"**R_v**: ${R_v:,.0f}")
    ...
    r_d_ok = params['R_d'] >= 0
    r_v_ok = R_v >= 0
    n_l_star_ok = sponsor_tvl['N_l_star'] >= 0
    n_d_star_ok = sponsor_tvl['N_d_star'] >= 0
    liquidity_budget_ok = (sponsor_tvl['N_l_star'] + sponsor_tvl['N_d_star']) <= params['N']
    
    # Calculate budget constraint
    budget_lhs = (params['R_l'] + params['R_d'] + R_v) - sponsor_tvl['N_l_star'] * params['r_b'] * (1 - params['epsilon']) - sponsor_tvl['N_d_star'] * params['r'] / 2
    budget_constraint_ok = abs(budget_lhs - params['R']) < 1e-6
    
    st.write(f"**R_l ≥ 0**: {'✅' if r_l_ok else '❌'} ({params['R_l']:,})")
    st.write(f"**R_d ≥ 0**: {'✅' if r_d_ok else '❌'} ({params['R_d']:,})")
    st.write(f"**R_v ≥ 0**: {'✅' if r_v_ok else '❌'} ({R_v:,.0f})")
    st.write(f"**N_l* ≥ 0**: {'✅' if n_l_star_ok else '❌'} ({sponsor_tvl['N_l_star']:,.0f})")
    st.write(f"**N_d* ≥ 0**: {'✅' if n_d_star_ok else '❌'} ({sponsor_tvl['N_d_star']:,.0f})")
    st.write(f"**N_l* + N_d* = N**: {'✅' if liquidity_budget_ok else '❌'} ({sponsor_tvl['N_l_star'] + sponsor_tvl['N_d_star']:,.0f} = {params['N']:,})")
    st.write(f"**Budget constraint**: {'✅' if budget_constraint_ok else '❌'} ({budget_lhs:,.0f} = {params['R']:,})")
